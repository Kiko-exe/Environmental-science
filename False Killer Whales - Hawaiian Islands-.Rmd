...
animal <- read.csv('~/Downloads/False Killer Whales - Hawaiian Islands- PIFSC.csv') # name the file as animal
...
# 1. 添加年份列
animal_sf$year <- year(animal_sf$timestamp)
...
for (y in years) {
  ...
  # a. 筛选该年份的轨迹数据
  animal_year <- animal_sf %>% filter(year == y)
  
  # b. 读取该年份的 SST 文件
  sst_file <- paste0("AQUA_MODIS.", y, "0101_", y, "1231.L3m.YR.SST.sst.4km.nc")
  if (!file.exists(sst_file)) {
    cat("SST file not found for year", y, "\n")
    next
  }
  sst <- rast(sst_file)
  
  # c. 裁剪 SST 图层到轨迹范围（加缓冲）
  bbox <- ext(vect(animal_year)) + 2
  sst_crop <- crop(sst, bbox)
  sst_df <- as.data.frame(sst_crop, xy = TRUE, na.rm = TRUE)
  
  # d. 绘图
  p <- ggplot() +
    geom_raster(data = sst_df, aes(x = x, y = y, fill = sst)) +
    scale_fill_viridis_c(option = "plasma", name = "SST (°C)", na.value = "transparent") +
    geom_sf(data = animal_year, aes(color = individual.local.identifier), 
            size = 1.2, shape = 21, stroke = 0.3) +  # 清晰显示每个点
    theme_minimal() +
    labs(title = paste("Whale Tracks and SST in", y),
         x = "Longitude", y = "Latitude", color = "Whale ID") +
...
# Step 1: 读取 SST 数据
sst <- rast("AQUA_MODIS.20020101_20021231.L3m.YR.SST.sst.4km.nc")

# Step 2: 读取你的轨迹数据（假设已经处理为 sf 对象）
# 示例：animal_sf <- st_as_sf(animal, coords = c("location.long", "location.lat"), crs = 4326)

# Step 3: 裁剪 SST 到轨迹范围 + 缓冲区
bbox <- ext(vect(animal_sf)) + 2
sst_crop <- crop(sst, bbox)

# Step 4: 转换为数据框用于 ggplot
sst_df <- as.data.frame(sst_crop, xy = TRUE, na.rm = TRUE)

# Step 5: 绘图
ggplot() +
  geom_raster(data = sst_df, aes(x = x, y = y, fill = sst)) +
  scale_fill_viridis_c(option = "plasma", name = "SST (°C)") +
  geom_sf(data = animal_sf, aes(color = individual.local.identifier), size = 0.4) +
  theme_minimal() +
  labs(title = "False Killer Whale Movements over Sea Surface Temperature (2002)",
       x = "Longitude", y = "Latitude", color = "Whale ID")
```
...
```{r}
chl_file <- "ESACCI-OC-L3S-OC_PRODUCTS-MERGED-1M_MONTHLY_4km_GEO_PML_OCx_QAA-201306-fv6.0.nc"
```

```{r}
chl_brick <- rast(chl_file)
names(chl_brick)
chl <- chl_brick[["chlor_a"]]

```

```{r}
animal_vect <- vect(animal_sf)
```

```{r}
bbox <- ext(animal_vect) + 2  # 添加2度缓冲
chl_crop <- crop(chl, bbox)
plot(chl_crop)
```

```{r}
animal_vect$chlor_a <- extract(chl_crop, animal_vect)[,2] 
animal_sf$chlor_a <- animal_vect$chlor_a
ggplot(animal_sf, aes(x = chlor_a, y = speedKmH)) +
  geom_point(alpha = 0.4, color = "steelblue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
  theme_bw() +
  labs(title = "Chlorophyll vs Whale Speed",
...
library(sf)
library(lubridate)
library(geosphere)  # 用于计算球面距离

# 提取坐标
animal_sf$lon <- st_coordinates(animal_sf)[,1]
animal_sf$lat <- st_coordinates(animal_sf)[,2]

# 转换时间格式（你的是字符型）
animal_sf$timestamp <- ymd_hms(animal_sf$timestamp)
```
...
animal_sf <- animal_sf %>%
  ...
  mutate(
    dist_m = distHaversine(matrix(c(lag(lon), lag(lat)), ncol = 2),
                           matrix(c(lon, lat), ncol = 2)),
    dt_sec = as.numeric(difftime(timestamp, lag(timestamp), units = "secs"))
  ) %>%
  ungroup()
```